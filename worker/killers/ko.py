...
async def run(ctx: BrowserContext, card_input: str, chat_id: int, message_id: int) -> bool:
    ...
    try:
        ...
        # Cookie banner
        try:
            await page.wait_for_selector("a.wscrOk", timeout=5000)
        ...
        # Step 4 — CVV loop (fast: 5 attempts)
        used = {wrong_cvv}
        for _ in range(5):
        ...
        await tg_admin_screenshot("ko", trace, screenshot)
        return False
    finally:
        await page.close()
