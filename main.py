...
async def health_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 4) Playwright worker health check
    worker_url = os.environ.get("WORKER_URL", "").rstrip("/")
    worker_ok = False
    worker_info = "not configured"
    if worker_url:
        try:
            async with httpx.AsyncClient(timeout=5) as client:
                ...
                if resp.status_code == 200:
                    ...
                    pool = data.get("pool", {})
                    worker_ok = True
                    worker_info = f"free={pool.get('free', '?')}/{pool.get('total', '?')} queue={data.get('queue_depth', '?')}"
        except Exception as e:
            worker_info = str(e)[:80]
    ...
    lines = [
        ...
        f"⚙️ *Worker:* {'✅' if worker_ok else '❌'} `{worker_info}`",
    ]
    ...
    lines.append("")
    lines.append("🔧 *Admin:* `/health reset`")
